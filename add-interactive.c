...
int init_add_i_state(struct repository *r, struct add_i_state *state)
{
	...
	init_color(r, state, "help", state->help_color, GIT_COLOR_BOLD_RED);
	init_color(r, state, "prompt", state->prompt_color,
		   GIT_COLOR_BOLD_BLUE);
	init_color(r, state, "error", state->error_color,
		   GIT_COLOR_BOLD_RED);
	...
	strlcpy(state->context_color,
		diff_get_color(state->use_color, DIFF_CONTEXT), COLOR_MAXLEN);
	strlcpy(state->file_old_color,
		diff_get_color(state->use_color, DIFF_FILE_OLD), COLOR_MAXLEN);
	...
	if (git_config_get_bool("interactive.singlekey",
				&state->use_single_key))
		state->use_single_key = 0;
...
struct list_and_choose_options {
	...
	void (*print_help)(struct add_i_state *state);
};
			       ...
			       struct list_and_choose_options *opts)
{
	...
	const char *prompt_color = s->prompt_color;
	const char *error_color = s->error_color;
	...
	if (unmerged_count || binary_count) {
		for (i = j = 0; i < files->nr; i++)
			if (files->file[i]->index.binary ||
				...
				color_fprintf_ln(stderr, error_color,
...
static void choose_prompt_help(struct add_i_state *state)
{
	const char *help_color = state->help_color;
...
static void command_prompt_help(struct add_i_state *state)
{
	const char *help_color = state->help_color;
