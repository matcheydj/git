...
static int parse_diff(struct add_p_state *state, const struct pathspec *ps)
{
	...
	while (p != pend) {
		...
		if (starts_with(p, "diff ")) {
			...
			if (deleted)
				file_diff->deleted = 1;
			else if (parse_hunk_header(state, hunk) < 0)
				return -1;
...
static void render_diff_header(struct add_p_state *state,
			       ...
			       struct strbuf *out)
{
	...
	if (colored) {
		const char *p = state->colored.buf;
...
static int merge_hunks(struct add_p_state *state, struct file_diff *file_diff,
		       size_t *hunk_index, int use_all, struct hunk *temp)
{
	...
	for (; i + 1 < file_diff->hunk_nr; i++) {
		...
		if (temp->start < hunk->start && temp->end > hunk->start) {
			...
			const char *plain = state->plain.buf;
			...
			for (i = 0; i < overlapping_line_count; i++) {
				overlap_next = find_next_line(&state->plain,
							      overlap_end);
			 ...
			 */
			if (temp->end != state->plain.len) {
				size_t start = state->plain.len;

				strbuf_add(&state->plain, plain + temp->start,
					   temp->end - temp->start);
				plain = state->plain.buf;
				temp->start = start;
				temp->end = state->plain.len;
			}

			strbuf_add(&state->plain,
				   plain + overlap_end,
...
{
	struct hunk *hunk;
	size_t save_len = state->plain.len, i;
	ssize_t delta = 0;

	render_diff_header(state, file_diff, 0, out);
	...
	for (i = file_diff->mode_change; i < file_diff->hunk_nr; i++) {
		...
		if (!use_all && hunk->use != USE_HUNK)
			...
			if (merge_hunks(state, file_diff, &i, use_all, &temp))
				hunk = &temp;
			 ...
			 */
			strbuf_setlen(&state->plain, save_len);
...
static int split_hunk(struct add_p_state *state, struct file_diff *file_diff,
		       size_t hunk_index)
{
	int colored = !!state->colored.len, first = 1;
...
next_hunk_line:
			...
			marker = ch;
			current = find_next_line(&state->plain, current);
			if (colored)
				colored_current =
					find_next_line(&state->colored,
						       colored_current);
			continue;
		}
...
static void recolor_hunk(struct add_p_state *state, struct hunk *hunk)
{
	const char *plain = state->plain.buf;
	...
	hunk->colored_start = state->colored.len;
	for (current = hunk->start; current < hunk->end; ) {
		...
		strbuf_addstr(&state->colored,
			      plain[current] == '-' ?
			      state->state.file_old_color :
			      plain[current] == '+' ?
			      state->state.file_new_color :
			      state->state.context_color);
		strbuf_add(&state->colored, plain + current, eol - current);
		strbuf_addstr(&state->colored, GIT_COLOR_RESET);
		if (next > eol)
			strbuf_add(&state->colored, plain + eol, next - eol);
		current = next;
	}
	hunk->colored_end = state->colored.len;
}

static int edit_hunk_manually(struct add_p_state *state, struct hunk *hunk)
{
	...
	strbuf_commented_addf(&buf, _("Manual hunk edit mode -- see bottom for "
				      "a quick guide.\n"));
	render_hunk(state, hunk, 0, 0, &buf);
	...
	hunk->start = state->plain.len;
	if (launch_editor(path, &state->plain, NULL) < 0) {
	...
	copy = state->plain.buf[hunk->start] != comment_line_char;
	for (i = j = hunk->start; i < state->plain.len; ) {
		if (copy)
			state->plain.buf[j++] = state->plain.buf[i];
		if (state->plain.buf[i++] == '\n')
			copy = state->plain.buf[i] != comment_line_char;
	}
	...
	res = 1;
	strbuf_setlen(&state->plain, j);
	hunk->end = j;
	recolor_hunk(state, hunk);
	if (state->plain.buf[hunk->start] == '@' &&
	    /* If the hunk header was deleted, simply use the original one. */
	    parse_hunk_header(state, hunk) < 0)
		res = -1;
...
static ssize_t recount_edited_hunk(struct add_p_state *state, struct hunk *hunk,
				   size_t orig_old_count, size_t orig_new_count)
{
	...
	for (i = hunk->start; i < hunk->end; ) {
		...
		i = find_next_line(&state->plain, i);
	}
...
static int run_apply_check(struct add_p_state *state,
...
static int edit_hunk_loop(struct add_p_state *state,
			  struct file_diff *file_diff, struct hunk *hunk)
{
	size_t plain_len = state->plain.len, colored_len = state->colored.len;
	struct hunk backup;

	memcpy(&backup, hunk, sizeof(backup));

	for (;;) {
		int res = edit_hunk_manually(state, hunk);
		if (res == 0) {
			/* abandonded */
		...
		if (res > 0) {
			hunk->delta +=
				recount_edited_hunk(state, hunk,
						    backup.header.old_count,
						    backup.header.new_count);
			if (!run_apply_check(state, file_diff))
				return 0;
		}

		/* Drop edits (they were appended to state->plain) */
		strbuf_setlen(&state->plain, plain_len);
		strbuf_setlen(&state->colored, colored_len);
		memcpy(hunk, &backup, sizeof(backup));
...
static size_t display_hunks(struct add_p_state *state,
			    struct file_diff *file_diff, size_t start_index)
{
	...
	while (start_index < end_index) {
		...
		strbuf_reset(&state->buf);
		strbuf_addf(&state->buf, "%c%2d: ", hunk->use == USE_HUNK ? '+'
			    : hunk->use == SKIP_HUNK ? '-' : ' ',
			    (int)start_index);
		summarize_hunk(state, hunk, &state->buf);
			     ...
			     struct file_diff *file_diff)
{
	...
	char ch;
	struct child_process cp = CHILD_PROCESS_INIT;
	int colored = !!state->colored.len, quit = 0;
	...
	strbuf_reset(&state->buf);
	render_diff_header(state, file_diff, colored, &state->buf);
	fputs(state->buf.buf, stdout);
	for (;;) {
		if (hunk_index >= file_diff->hunk_nr)
			hunk_index = 0;
...
soft_increment:
			...
			strbuf_trim(&state->answer);
			i = hunk_index > 10 ? hunk_index - 10 : 0;
			while (state->answer.len == 0) {
				i = display_hunks(state, file_diff, i);
			...
			strbuf_trim(&state->answer);
			response = strtoul(state->answer.buf, &pend, 10);
			if (*pend || pend == state->answer.buf)
				color_fprintf_ln(stderr,
						 state->state.error_color,
						 _("Invalid number: '%s'"),
						 state->answer.buf);
			...
			if (splittable_into < 2)
				color_fprintf_ln(stderr,
						 state->state.error_color,
						 _("Sorry, cannot split this "
						   "hunk"));
			else if (!split_hunk(state, file_diff,
					     hunk - file_diff->hunk))
				color_fprintf_ln(stdout,
						 state->state.header_color,
						 _("Split into %d hunks."),
						 (int)splittable_into);
		} else if (state->answer.buf[0] == 'e') {
			if (hunk_index + 1 == file_diff->mode_change)
				color_fprintf_ln(stderr,
						 state->state.error_color,
						 _("Sorry, cannot edit this "
						   "hunk"));
			else if (edit_hunk_loop(state, file_diff, hunk) >= 0) {
				hunk->use = USE_HUNK;
				goto soft_increment;
			}
		} else {
			...
			for (; *p; p = eol + (*eol == '\n')) {
				...
				/*
				 * `state->buf` still contains the part of the
				 * commands shown in the prompt that are not
				 * always available.
				 */
				if (*p != '?' && !strchr(state->buf.buf, *p))
					continue;

				color_fprintf_ln(stdout,
						 state->state.help_color,
						 "%.*s", (int)(eol - p), p);
			}
		}
	}
